...
const firstname = ({gender = ''} = {}) => {
  ...
  if (gender === 'f')
    return list({values: firstnamesFemale});
...
const email = ({fullname = '', prefix = 'fake.', domain = ''} = {}) => {
  if (!fullname)
    fullname = fullname();
  ...
  const sep = domain ? '.' : list({values: ['.', '-', '']});
  fullname = `${firstname.replace(/[^\w]/, '')}${sep}${lastname.replace(/[^\w]/, '')}`;
  if (!domain && boolean({percent: 0.75}))
    fullname += integer({min: 12, max: 88});

  if (!domain)
    domain = list({values: emailDomains});

  return `${fullname}@${prefix}${domain}`;
}

export { firstname, lastname, fullname, email }
